...
/**
    ...
    @param headers Вектор с заголовком для запроса
    @retrun Строка с ответом от сервера или пустая строка
    @note работает только на винде 
*/ 
...
public:
    /**
        @brief Конструктор класса, принимает ответы сервера для обработки
    */
    SimpleJsonParser(const string& str) : json(str) {}

    /**
        @brief Парсим массив объектов и извлекаем значения subject_name
        @return словарь с отсутствующими данными по расписанию
    */
...
int main() {
    ...
    vector<string> headers{ "referer: https://journal.top-academy.ru/" };
    cout << "Пришлите токен Bearer: ";
    string Bearer;
    cin >> Bearer;
    headers.push_back(("Authorization: Bearer " + Bearer));
    cout << "Запрос на api..." << url << "..." << endl;
